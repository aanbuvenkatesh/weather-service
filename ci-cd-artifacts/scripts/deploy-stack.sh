...
if [[ "ci-cd-artifacts/"$TEMPLATE_NAME != *.yaml ]]; then
    echo "CloudFormation template $TEMPLATE_NAME does not exist. Make sure the extension is *.yaml and not (*.yml)"
    exit 0
fi

if [[ "ci-cd-artifacts/parameters/"$PARAMETERS_FILE_NAME != *.properties ]]; then
...
if [[ $CHANGESET_MODE == "true" ]] || [[ $CHANGESET_MODE == "True" ]]; then
    ...
    --template-file ci-cd-artifacts/$TEMPLATE_NAME \
    --parameter-overrides file://ci-cd-artifacts/parameters/$PARAMETERS_FILE_NAME \
